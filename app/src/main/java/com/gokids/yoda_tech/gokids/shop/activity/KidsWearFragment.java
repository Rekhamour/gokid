...
public class KidsWearFragment extends Fragment {
    ...
    public int getTotalRestaurants(final String category) {
        Ion.with(getActivity())
                .load(Urls.BASE_URL+"api/categoryTotalCount/category/CLS2/subCategory/CAT2"+"/city/"+ MySharedPrefrence.getPrefrence(ctx).getString("current_city",""))
                .asJsonObject()
    ...
    public static ArrayList<MainBean> getRestaurants(final String category, final String name, final double lat, final double longi, final String sortBy, final int start, final int count){
        ...
        String PATH= Urls.BASE_URL + "api/viewAllShops/latitude/"+lat+"/longitude/"+longi+"/category/CAT2/limitStart/"+ start+"/count/"+(start+50)+"/sortBy/Distance/searchBy/"+category+"/city/"+ MySharedPrefrence.getPrefrence(ctx).getString("current_city","");
        Log.e(TAG,"path"+PATH);

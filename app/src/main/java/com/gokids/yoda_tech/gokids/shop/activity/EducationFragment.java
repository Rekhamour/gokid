...
public class EducationFragment extends Fragment  {
    ...
    public int getTotalRestaurants(final String category) {
        Ion.with(getActivity())
                .load(Urls.BASE_URL+"api/categoryTotalCount/category/CLS2/subCategory/CAT5"+"/city/"+ MySharedPrefrence.getPrefrence(ctx).getString("current_city",""))
                .asJsonObject()
    ...
    public static ArrayList<MainBean> getRestaurants(final String category, final String name, final double lat, final double longi, final String sortBy, final int start, final int count){
        ...
        String PATH= Urls.BASE_URL + "api/viewAllShops/latitude/"+lat+"/longitude/"+longi+"/category/CAT5/limitStart/"+ start+"/count/"+(start+50)+"/sortBy/Distance/searchBy/"+category+"/city/"+ MySharedPrefrence.getPrefrence(ctx).getString("current_city","");
        Log.e(TAG,"path"+PATH);
                                ...
                                if (result.has("result")) {
                                    if (res.size() > 0) {
                                        for (int i = 0; i < res.size(); i++) {
                                            ...
                                            if(obj.getAsJsonObject().has(".isJsonArray()")) {
                                                if (obj.getAsJsonObject().get("C").isJsonArray()) {

package com.gokids.yoda_tech.gokids.eat.model;

import java.io.Serializable;
import java.util.ArrayList;

/**
 * Created by rigoe on 6/2/2017.
 */

public class MainBean implements Serializable{
    private String ShopID;
    private String RestaurantID;
    private String EntertainmentID;
    private String EntertainmentTitle;
    private String Entertainmentdetail;
    private String ShopDetail;
    private String ShopName;
    private String ShopSubName;
    private String StartDate;
    private String EndDate;
    private String EventDate;
    private String Classname;
    private String medicalID;
    private String category;
    private String RestaurantName;
    private String RestaurantSubName;
    private String Specialty;
    private String WorkingHour;
    private String Website;
    private String doctorDetail;
    private String email;
    private String procedure;
    private String Price;
    private String schedule;
    private String frequency;
    private String image;
    private String medicalId;
    private String name;
    private String address;
    private String postal;
    private String latlong;
    private int kidsfinityScore;
    private String distance;
    private String yearOfExp;
    private String offer;
    private ArrayList<CuisinesBean> Cuisines;
    private ArrayList<Specialization> specializations;
    private ArrayList<String> images;
    private ArrayList<Contact> contacts;
    private String tags;
    private String rates;
    private String mySchedule;
    private String LatLong;

    public String getLatLong() {
        return LatLong;
    }

    public void setLatLong(String latLong) {
        LatLong = latLong;
    }

    public String getRestaurantID() {
        return RestaurantID;
    }

    public void setRestaurantID(String restaurantID) {
        RestaurantID = restaurantID;
    }

    public String getRestaurantName() {
        return RestaurantName;
    }

    public void setRestaurantName(String restaurantName) {
        RestaurantName = restaurantName;
    }

    public String getRestaurantSubName() {
        return RestaurantSubName;
    }

    public void setRestaurantSubName(String restaurantSubName) {
        RestaurantSubName = restaurantSubName;

    }

    public String getShopID() {
        return ShopID;
    }

    public void setShopID(String shopID) {
        ShopID = shopID;
    }

    public String getSpecialty() {
        return Specialty;
    }

    public void setSpecialty(String specialty) {
        Specialty = specialty;
    }

    public String getWorkingHour() {
        return WorkingHour;
    }

    public void setWorkingHour(String workingHour) {
        WorkingHour = workingHour;
    }

    public String getPrice() {
        return Price;
    }

    public void setPrice(String price) {
        Price = price;
    }

    public ArrayList<CuisinesBean> getCuisines() {
        return Cuisines;
    }

    public void setCuisines(ArrayList<CuisinesBean> cuisines) {
        Cuisines = cuisines;
    }

    public String getWebsite() {
        return Website;
    }

    public void setWebsite(String website) {
        Website = website;
    }

    public String getDoctorDetail() {
        return doctorDetail;
    }

    public void setDoctorDetail(String doctorDetail) {
        this.doctorDetail = doctorDetail;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String getProcedure() {
        return procedure;
    }

    public void setProcedure(String procedure) {
        this.procedure = procedure;
    }


    public String getSchedule() {
        return schedule;
    }

    public void setSchedule(String schedule) {
        this.schedule = schedule;
    }

    public String getFrequency() {
        return frequency;
    }

    public void setFrequency(String frequency) {
        this.frequency = frequency;
    }

    public String getImage() {
        return image;
    }

    public void setImage(String image) {
        this.image = image;
    }

    public String getMedicalId() {
        return medicalId;
    }

    public void setMedicalId(String medicalId) {
        this.medicalId = medicalId;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getAddress() {
        return address;
    }

    public void setAddress(String address) {
        this.address = address;
    }

    public String getPostal() {
        return postal;
    }

    public void setPostal(String postal) {
        this.postal = postal;
    }

    public String getLatlong() {
        return latlong;
    }

    public void setLatlong(String latlong) {
        this.latlong = latlong;
    }

    public int getKidsfinityScore() {
        return kidsfinityScore;
    }

    public void setKidsfinityScore(int kidsfinityScore) {
        this.kidsfinityScore = kidsfinityScore;
    }

    public String getDistance() {
        return distance;
    }

    public void setDistance(String distance) {
        this.distance = distance;
    }

    public String getYearOfExp() {
        return yearOfExp;
    }

    public void setYearOfExp(String yearOfExp) {
        this.yearOfExp = yearOfExp;
    }

    public String getOffer() {
        return offer;
    }

    public void setOffer(String offer) {
        this.offer = offer;
    }

    public ArrayList<Specialization> getSpecializations() {
        return specializations;
    }

    public void setSpecializations(ArrayList<Specialization> specializations) {
        this.specializations = specializations;
    }

    public ArrayList<String> getImages() {
        return images;
    }

    public void setImages(ArrayList<String> images) {
        this.images = images;
    }

    public ArrayList<Contact> getContacts() {
        return contacts;
    }

    public void setContacts(ArrayList<Contact> contacts) {
        this.contacts = contacts;
    }

    public String getTags() {
        return tags;
    }

    public void setTags(String tags) {
        this.tags = tags;
    }

    public String getRates() {
        return rates;
    }

    public void setRates(String rates) {
        this.rates = rates;
    }

    public String getMySchedule() {
        return mySchedule;
    }

    public String getMedicalID() {
        return medicalID;
    }

    public void setMedicalID(String medicalID) {
        this.medicalID = medicalID;
    }

    public String getCategory() {
        return category;
    }

    public void setCategory(String category) {
        this.category = category;
    }

    public String getClassname() {
        return Classname;
    }

    public void setClassname(String classname) {
        Classname = classname;
    }

    public String getEntertainmentID() {
        return EntertainmentID;
    }

    public void setEntertainmentID(String entertainmentID) {
        EntertainmentID = entertainmentID;
    }

    public String getShopSubName() {
        return ShopSubName;
    }

    public void setShopSubName(String shopSubName) {
        ShopSubName = shopSubName;
    }

    public String getShopName() {
        return ShopName;
    }

    public void setShopName(String shopName) {
        ShopName = shopName;
    }

    public String getEntertainmentdetail() {
        return Entertainmentdetail;
    }

    public void setEntertainmentdetail(String entertainmentdetail) {
        Entertainmentdetail = entertainmentdetail;
    }

    public String getEntertainmentTitle() {
        return EntertainmentTitle;
    }

    public void setEntertainmentTitle(String entertainmentTitle) {
        EntertainmentTitle = entertainmentTitle;
    }

    public String getShopDetail() {
        return ShopDetail;
    }

    public void setShopDetail(String shopDetail) {
        ShopDetail = shopDetail;
    }

    public String getStartDate() {
        return StartDate;
    }

    public void setStartDate(String startDate) {
        StartDate = startDate;
    }

    public String getEndDate() {
        return EndDate;
    }

    public void setEndDate(String endDate) {
        EndDate = endDate;
    }

    public String getEventDate() {
        return EventDate;
    }

    public void setEventDate(String eventDate) {
        EventDate = eventDate;
    }

    public void setMySchedule(String mySchedule) {
        this.mySchedule = mySchedule;
    }
}
